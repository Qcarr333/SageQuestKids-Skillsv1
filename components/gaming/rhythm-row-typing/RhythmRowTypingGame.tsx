...
export function RhythmRowTypingGame(){
  ...
  const keyboardTargets=useMemo(
    () => Array.from(new Set(task.prompt_sequence.join('').replace(/\s/g, '').split(''))).map((key) => ({ key })),
    [task.prompt_sequence]
  );
